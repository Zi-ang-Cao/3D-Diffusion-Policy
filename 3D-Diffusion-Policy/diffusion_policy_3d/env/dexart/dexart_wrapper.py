...
class DexArtEnv(gym.Env):
    ...
    def __init__(self, task_name, use_test_set=False, num_points=1024):
        ...
        self.action_space = spaces.Box(
            low=-1, high=1, shape=(robot_dof,), dtype=np.float32
        )
        self.obs_sensor_dim = 32
        self.num_points = num_points
        self.observation_space = spaces.Dict(
            {
                "image": spaces.Box(low=0, high=1, shape=(3, 84, 84), dtype=np.float32),
                ...
                "imagin_robot": spaces.Box(
                    low=-np.inf, high=np.inf, shape=(96, 7), dtype=np.float32
                ),
            }
        )
    ...
    def step(self, action):
        ...
        if obs_pointcloud.shape[0] > self.num_points:
            obs_pointcloud = downsample_with_fps(obs_pointcloud, self.num_points)
