...
def point_cloud_sampling(
    point_cloud: np.ndarray, num_points: int, method: str = "uniform"
):
    """
    ...
    if point_cloud.shape[0] <= num_points:
        ...
        point_cloud = np.concatenate(
            [point_cloud, np.zeros((num_points - point_cloud.shape[0], 6))], axis=0
        )
        return point_cloud
...
class MujocoPointcloudWrapperAdroit(gym.Wrapper):
    ...
    def __init__(self, env, env_name: str, use_point_crop=True):
        ...
        self.point_sampling_method = ENV_POINT_CLOUD_CONFIG[env_name].get(
        ...
        assert self.point_sampling_method in [
            "uniform",
            "fps",
