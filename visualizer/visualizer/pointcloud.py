...
class Visualizer:
    ...
    def _generate_trace(self, pointcloud, color: tuple = None, size=5, opacity=0.7):
        ...
        if pointcloud.shape[1] == 3:
            if color is None:
                ...
                normalized_coords = (pointcloud - min_coords) / (
        ...
        return go.Scatter3d(
            ...
            z=z_coords,
            mode="markers",
            marker=dict(size=size, opacity=opacity, color=colors),
        )
    ...
    def visualize_pointcloud_and_save(
        self, pointcloud, color: tuple = None, save_path=None
    ):
        trace = self._generate_trace(pointcloud, color=color, size=6, opacity=1.0)
        layout = go.Layout(margin=dict(l=0, r=0, b=0, t=0))
        fig = go.Figure(data=[trace], layout=layout)
