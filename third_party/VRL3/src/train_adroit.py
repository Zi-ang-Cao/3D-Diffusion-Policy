...
def print_stage2_time_est(time_used, curr_n_update, total_n_update):
    ...
    print(
        "Stage 2 [{:.2f}%]. Frames:[{:.0f}/{:.0f}]K. Time:[{:.2f}/{:.2f}]hrs. Overall FPS: {}.".format(
            curr_n_update / total_n_update * 100,
            curr_n_update / 1000,
            total_n_update / 1000,
            time_used / 3600,
            est_total_time / 3600,
            int(curr_n_update / time_used),
        )
    )
...
def print_stage3_time_est(time_used, curr_n_frames, total_n_frames):
    ...
    est_time_remaining = est_total_time - time_used
    print(
        "Stage 3 [{:.2f}%]. Frames:[{:.0f}/{:.0f}]K. Time:[{:.2f}/{:.2f}]hrs. Overall FPS: {}.".format(
            curr_n_frames / total_n_frames * 100,
            curr_n_frames / 1000,
            total_n_frames / 1000,
            time_used / 3600,
            est_total_time / 3600,
            int(curr_n_frames / time_used),
        )
    )
...
class Workspace:
    def __init__(self, cfg):
        ...
        if (
            self.cfg.debug > 0
        ):  # if debug mode, then change hyperparameters for quick testing
    ...
    def set_debug_hyperparameters(self):
        self.cfg.num_seed_frames = (
            1000 if self.cfg.num_seed_frames > 1000 else self.cfg.num_seed_frames
        )
        self.cfg.agent.num_expl_steps = (
            500
            if self.cfg.agent.num_expl_steps > 1000
            else self.cfg.agent.num_expl_steps
        )
    ...
    def setup(self):
        ...
        env_type = (
            "adroit"
            if env_name in ("hammer-v0", "door-v0", "pen-v0", "relocate-v0")
            else "dmc"
        )
        ...
        self.replay_loader = make_replay_loader(
            ...
            self.cfg.nstep,
            self.cfg.discount,
            self.replay_buffer_fetch_every,
            is_adroit=IS_ADROIT,
        )
    ...
    def eval_adroit(self, force_number_episodes=None, do_log=True):
        ...
        step, episode, total_reward = 0, 0, 0
        n_eval_episode = (
            force_number_episodes
            if force_number_episodes is not None
            else self.cfg.eval_episodes
        )
    ...
    def load_demo(self, replay_storage, env_name, verbose=True):
        ...
        for i_path in range(len(demo_data)):
            ...
            if verbose:
                print(
                    "demo trajectory %d, len: %d, return: %.2f, goal achieved steps: %d"
    ...
    def get_pretrained_model_path(self, stage1_model_name):
        ...
        model_path = os.path.join(
            model_folder_path, stage1_model_name + "_checkpoint.pth.tar"
        )
        return model_path
    ...
    def train(self):
        ...
        if self.cfg.stage1_use_pretrain:
            self.agent.load_pretrained_encoder(
                self.get_pretrained_model_path(self.cfg.stage1_model_name)
            )
        ...
        print(
            "Model and data loading finished in %.2f hours."
        ...
        if stage2_n_update > 0:
            for i_stage2 in range(stage2_n_update):
                metrics = self.agent.update(
                    self.replay_iter, i_stage2, stage=2, use_sensor=IS_ADROIT
                )
                if i_stage2 % self.cfg.stage2_eval_every_frames == 0:
                    average_score, succ_rate = self.eval_adroit(
                        force_number_episodes=self.cfg.stage2_eval_episodes,
                        do_log=False,
                    )
                    print(
                        "Stage 2 step %d, Q(s,a): %.2f, Q loss: %.2f, score: %.2f, succ rate: %.2f"
                        % (
                            i_stage2,
                            metrics["critic_q1"],
                            metrics["critic_loss"],
                ...
                if (
                    self.cfg.show_computation_time_est
                    and i_stage2 > 0
                    and i_stage2 % self.cfg.show_time_est_interval == 0
                ):
                    print_stage2_time_est(
                        time.time() - stage2_start_time, i_stage2 + 1, stage2_n_update
                    )
        print(
            "Stage 2 finished in %.2f hours."
            % ((time.time() - stage2_start_time) / 3600)
        )
        ...
        while train_until_step(self.global_step):
            ...
            if self.global_step % 1000 == 0 and metrics is not None:
                ...
                episode_frame_since_log = (
                    episode_step_since_log * self.cfg.action_repeat
                )
                with self.logger.log_and_dump_ctx(self.global_frame, ty="train") as log:
                    ...
                    log("step", self.global_step)
                episode_step_since_log, episode_reward_list, episode_frame_list = (
                    0,
                    [0],
                    [0],
                )
            if (
                ...
                and self.global_step % self.cfg.show_time_est_interval == 0
            ):
                print_stage3_time_est(
                    time.time() - stage3_start_time,
                    self.global_frame + 1,
                    self.cfg.num_train_frames,
                )
            ...
            if not seed_until_step(self.global_step):
                metrics = self.agent.update(
                    self.replay_iter, self.global_step, stage=3, use_sensor=IS_ADROIT
                )
            ...
            if (
                (self.global_step == 1)
                or (self.global_step == 10000)
                or (self.global_step % 100000 == 0)
            ):
                try:
                    self.copy_to_azure()
        ...
        print(
            "Stage 3 finished in %.2f hours."
            % ((time.time() - stage3_start_time) / 3600)
        )
        print(
            "All stages finished in %.2f hrs. Work dir:"
            % ((time.time() - train_start_time) / 3600)
        )
        print(self.work_dir)
