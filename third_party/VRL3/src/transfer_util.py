...
def set_parameter_requires_grad(model, n_resblock_finetune):
    ...
    for name, param in model.named_parameters():
        condition = (
            (n_resblock_finetune >= 1 and "layer4" in name)
            or (n_resblock_finetune >= 2 and "layer3" in name)
            or (n_resblock_finetune >= 3 and "layer2" in name)
            or (n_resblock_finetune >= 4 and "layer1" in name)
            or (n_resblock_finetune >= 5)
        )
