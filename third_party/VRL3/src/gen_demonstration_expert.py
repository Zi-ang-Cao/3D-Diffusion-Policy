...
def parse_args():
    ...
    parser.add_argument(
        "--env_name", type=str, default="door", help="environment to run"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--expert_ckpt_path", type=str, default=None, help="path to expert ckpt"
    )
    parser.add_argument("--img_size", type=int, default=84, help="image size")
    parser.add_argument(
        "--not_use_multi_view", action="store_true", help="not use multi view"
    )
...
def main():
    ...
    loaded_dict = torch.load(
        args.expert_ckpt_path, map_location="cpu", weights_only=False
    )
    ...
    while episode_idx < num_episodes:
        ...
        if n_goal_achieved_total < 10.0:
            cprint(
                f"Episode {episode_idx} has {n_goal_achieved_total} goals achieved and {total_reward} reward. Discarding.",
                "red",
            )
        else:
            total_count += total_count_sub
            episode_ends_arrays.append(
                deepcopy(total_count)
            ...
            print(
                "Episode: {}, Reward: {}, Goal Achieved: {}".format(
                    episode_idx, total_reward, n_goal_achieved_total
                )
            )
            episode_idx += 1
