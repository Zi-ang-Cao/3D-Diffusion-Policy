    ...
    def __init__(self, log_dir, use_tb, stage2_logger=False):
        ...
        if not stage2_logger:
            ...
            self._eval_mg = MetersGroup(
                log_dir / "eval.csv", formating=COMMON_EVAL_FORMAT
            )
        else:
            self._train_mg = MetersGroup(
                log_dir / "train_stage2.csv", formating=COMMON_TRAIN_FORMAT
            )
            self._eval_mg = MetersGroup(
                log_dir / "eval_stage2.csv", formating=COMMON_EVAL_FORMAT
            )
        if use_tb:
