...
parser.add_argument(
    "--debug", default=0, type=int, help="1 for debug mode, 2 for super fast debug mode"
)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.debug > 0:
        ...
        model = ResNet84(
            BasicBlock, [0, 0, 0, 0], num_classes=5
        )  # 0, 0, 0, 0 make a convnet6 (5 conv layers in total lol)
        ...
        if args.pretrained:
            ...
            if args.arch in rl_model_names:
                if args.arch == "resnet18_32channel":
                    model = ResNet84(
                        BasicBlock, [2, 2, 2, 2], start_num_channel=32
                    )  # 1, 1, 1, 1 will make a resnet10
                elif args.arch == "resnet10_32channel":
                    model = ResNet84(
                        BasicBlock, [1, 1, 1, 1], start_num_channel=32
                    )  # 1, 1, 1, 1 will make a resnet10
                elif args.arch == "resnet6_32channel":
                    model = ResNet84(
                        BasicBlock, [0, 0, 0, 0], start_num_channel=32
                    )  # resnet 6 (actually not even resnet because no skip connection)
