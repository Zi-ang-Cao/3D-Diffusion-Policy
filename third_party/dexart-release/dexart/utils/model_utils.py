...
def build_free_root(
    ...
    joint_name = [f"{name}_joint" for name in "xyz"] + [
        f"{name}_rotation_joint" for name in "xyz"
    ]
    for i in range(6):
...
def build_ball_joint(
    robot_builder: sapien.ArticulationBuilder,
    parent: sapien.LinkBuilder,
    name: str,
    pose: Pose,
) -> List[sapien.ArticulationBuilder]:
    ...
    pose_in_parent = [
        pose,
        Pose(q=euler2quat(0, 0, np.pi / 2)),
        Pose(q=euler2quat(0, -np.pi / 2, 0)),
    ]
    pose_in_child = [
        Pose(),
        Pose(q=euler2quat(0, 0, np.pi / 2)),
    ...
    for i in range(3):
        ...
        link.set_joint_properties(
            "revolute",
            limits=joint_limit,
            pose_in_parent=pose_in_parent[i],
            pose_in_child=pose_in_child[i],
        )
    return links


def rot_from_connected_link(
    pos_parent, pos_child, parent_rotation=np.eye(3), up_axis=np.array([0, 1, 0])
):
...
def create_visual_material(
    renderer: sapien.VulkanRenderer, specular, metallic, roughness, base_color
):
    if renderer is None:
        return None
