...
class BucketRLEnv(BucketEnv, BaseRLEnv):
    def __init__(
        ...
        **renderer_kwargs,
        ...
        super().__init__(
            ...
            handle_type="left",
            fix_root_link=False,
            thick_handle=thick_handle,
            **renderer_kwargs,
        )
        self.box = self.create_box(
            sapien.Pose(p=np.array([-0.5, 0.0, 0.15])),
        ...
        self.robot_init_pose = sapien.Pose(
            np.array([-0.5, 0, 0.3]), transforms3d.euler.euler2quat(0, 0, 0)
        )
    ...
    def update_cached_state(self):
        ...
        self.strict_contact = (
            np.sum(self.robot_object_contact[:-1]) >= 3
        ...
        finger_contact_boolean = self.check_actor_pair_contacts(
            check_contact_links, self.instance_base_link
        )
        ...
        self.is_finger_touch_instance_base_percent = (
            np.sum(self.robot_instance_base_contact[:]) / 5
        )
        ...
        self.pose_mat = transforms3d.quaternions.quat2mat(
            self.instance.get_root_pose().q
        )
        ...
        if np.linalg.norm(self.palm_pose.p - self.handle_pose.p) > 0.2:  # Reaching
            ...
            self.state = 3
        self.early_done = (
            (self.progress > 0.9) and (self.state == 3) and (self.delta_height > 0.3)
        )
        self.is_eval_done = (self.progress > 0.7) and (
            self.delta_height > self.target_delta_height
        )
        self.last_palm_height = (
            self.palm_height if self.palm_height else self.palm_pose.p[2]
        )
        self.palm_height = self.palm_pose.p[2]
        self.last_grasp_dist = (
            self.grasp_dist
            if self.grasp_dist
            else (
    ...
    def get_reward(self, action):
        ...
        reward -= (
            0.2 * self.finger_base_touched_percent
        )  # under no circumstances should hand touch bucket base
        if self.state == 1:
            reward = -0.1 * min(
            ...
            reward -= 0.01 * np.linalg.norm(
                self.instance_base_link.get_angular_velocity()
            )
    ...
    def reset(
        ...
        *,
        ...
        self.init_pose_mat = transforms3d.quaternions.quat2mat(
            self.instance.get_root_pose().q
        )
        # ====================================================================
    ...
    def setup_robot_annotation(self, robot_name: str):
        ...
        self.pos_path = (
            ...
            / "assets"
            / "annotation"
            / f"bucket_{robot_name}_relative_position.json"
        )
