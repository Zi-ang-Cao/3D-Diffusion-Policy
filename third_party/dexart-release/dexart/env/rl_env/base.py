...
from dexart.utils.common_robot_utils import (
    load_robot,
...
class BaseRLEnv(BaseSimulationEnv, gym.Env):
    def __init__(
        ...
        **renderer_kwargs,
        ...
        self.imagination_data: Dict[
            str, Dict[str, Tuple[sapien.ActorBase, np.ndarray, int]]
        ] = {}
        self.need_flush_when_change_instance = False
        self.imaginations: Dict[str, np.ndarray] = {}
        self.eval_cam_names = (
            renderer_kwargs["eval_cam_names"]
            if renderer_kwargs.__contains__("eval_cam_names")
            else None
        )
        self.use_history_obs = (
            renderer_kwargs["use_history_obs"]
            if renderer_kwargs.__contains__("use_history_obs")
            else False
        )
        self.last_obs = None
    ...
    def setup(self, robot_name):
        ...
        if self.is_robot_free:
            ...
            velocity_limit = np.array(
                [1.0] * 3 + [1.57] * 3 + [3.14] * (self.robot.dof - 6)
            )
    ...
    def free_sim_step(self, action: np.ndarray):
        ...
        target_qpos = np.concatenate(
            [np.zeros(6), recover_action(action[6:], self.robot.get_qlimits()[6:])]
        )
    ...
    def arm_sim_step(self, action: np.ndarray):
        ...
        arm_qvel = compute_inverse_kinematics(target_root_velocity, palm_jacobian)[
            : self.arm_dof
        ]
        arm_qvel = np.clip(arm_qvel, -np.pi / 1, np.pi / 1)
        arm_qpos = (
            arm_qvel * self.control_time_step + self.robot.get_qpos()[: self.arm_dof]
        )
        ...
        self.cartesian_error = np.linalg.norm(
            relative_pos - target_root_velocity[:3] * self.control_time_step
        )
    ...
    def arm_kinematic_step(self, action: np.ndarray):
        ...
        arm_qpos = (
            arm_qvel * self.scene.timestep * self.frame_skip
            + self.robot.get_qpos()[: self.arm_dof]
        )
        target_qpos = np.concatenate(
            [
                arm_qpos,
                recover_action(action[6:], self.robot.get_qlimits()[self.arm_dof :]),
            ]
        )
    ...
    def configure_robot_contact_reward(self):
        ...
        self.palm_link = [
            link for link in self.robot.get_links() if link.get_name() == "base_link"
        ][0]
        # configure fingers
        finger_tip_names = [
            "link_15.0_tip",
        ...
        self.thumb_links = [
            robot_links[robot_link_names.index(name)] for name in thumb_link_name
        ]
        self.index_links = [
            robot_links[robot_link_names.index(name)] for name in index_link_name
        ]
        self.middle_links = [
            robot_links[robot_link_names.index(name)] for name in middle_link_name
        ]
        self.ring_links = [
            robot_links[robot_link_names.index(name)] for name in ring_link_name
        ]
        self.finger_tip_links = [
            robot_links[robot_link_names.index(name)] for name in finger_tip_names
        ]
        self.finger_contact_links = (
            self.thumb_links + self.index_links + self.middle_links + self.ring_links
        )
        ...
        self.arm_contact_links = [
            self.robot.get_links()[robot_link_names.index(name)]
            for name in arm_contact_link_name
        ]
        self.robot_object_contact = np.zeros(
            len(finger_tip_names) + 1
        )  # contact buffer of four tip and palm
        ...
        self.palm_id = [self.palm_link.get_id()] + [
            robot_links[robot_link_names.index("link_12.0")].get_id()
        ]
            ...
            for modality, cfg in camera_cfg.items():
                if modality == "point_cloud":
                    if "num_points" not in cfg:
                        raise RuntimeError(
                            f"Missing num_points in camera {name} point_cloud config."
                        )
        ...
        for img_type, link_config in config.items():
            ...
            if img_type == "robot":
                ...
                for link_name, point_size in link_config.items():
                    ...
                    link = [
                        link
                        for link in self.robot_collision_links
                        if link.get_name() == link_name
                    ][0]
                    o3d_mesh = actor_to_open3d_mesh(
                        link, use_collision_mesh=False, use_actor_pose=False
                    )
                ...
                for i, link in enumerate(self.instance_collision_links):
                    o3d_mesh = actor_to_open3d_mesh(
                        link, use_collision_mesh=True, use_actor_pose=False
                    )
                ...
                for i, link in enumerate(self.instance_collision_links):
                    ...
                    if i != len(self.instance_collision_links) - 1:
                        points[i] = int(
                            link_config["link_template"] * surface_weight[i]
                        )
                    else:
        ...
        for img_type, link_config in self.imagination_infos.items():
            ...
            if img_type == "instance":
                ...
                for i, link in enumerate(self.instance_collision_links):
                    ...
                    cloud_points = np.asarray(sampled_cloud.points)
                    self.imagination_data[img_type][link.get_name()] = (
                        link,
                        cloud_points,
                        3,
                    )
            ...
            if img_type == "goal":
                ...
                for link_name, (actor, points, img_class) in img_config.items():
                    ...
                    if link_name in [
                        ...
                        seg_vector[0, 2] = 1
                    seg_vector = np.repeat(
                        seg_vector, transformed_points.shape[0], axis=0
                    )  # Ni * 4
                    transformed_points = np.concatenate(
                        [transformed_points, seg_vector], axis=1
                    )  # Ni * 7
                ...
                self.imaginations[f"imagination_{img_type}"] = np.concatenate(
                    imagination_instance, axis=0
                )  # sum(Ni) * 7
    ...
    def get_visual_observation(self):
        ...
        if self.use_history_obs:
            ...
            if self.last_obs is None or len(self.last_obs.keys()) != 2 * len(
                current_obs
            ...
            self.last_obs = (
                current_obs  # self.last_obs does not contain the previous history
            )
        # =========================================================================
        return camera_obs
        ...
        for name, camera_cfg in self.camera_infos.items():
            ...
            for modality in modalities:
                if modality == "rgb":
                    ...
                    and camera_cfg["point_cloud"].get("use_seg") is True
                ):
            ...
            for modality in modalities:
                ...
                if (
                    ...
                    sapien.dlpack.dl_to_numpy_cuda_async_unchecked(
                        dl_tensor_pos, output_array_pos
                    )
                    sapien.dlpack.dl_cuda_sync()
                    obs_pos = np.reshape(output_array_pos[..., :3], (-1, 3))  # H, W, 3
                    obs_seg = np.reshape(output_array_seg[..., 1:2], (-1, 1))  # H, W, 1
                    ...
                    if obs_dict[f"{name}-seg_gt"].shape != (
                        camera_cfg["point_cloud"]["num_points"],
                        4,
                    ):
                        # align the gt segmentation mask
                        obs_dict[f"{name}-seg_gt"] = np.zeros(
                            (camera_cfg["point_cloud"]["num_points"], 4)
                        )
                    obs = obs[:, :6]
                else:
                    dl_tensor = dl_list[i]
                    ...
                    if modality == "segmentation":
                        ...
                        output_array = (
                            torch.from_dlpack(dl_tensor).cpu().numpy()
                        )  # H, W, 4. [..., 0]: mesh-level segmentation; [..., 1]: link-level segmentation
                    elif modality == "rgb":
                        import torch
    ...
    def observation_space(self):
        ...
        if not self.use_visual_obs:
            ...
            oracle_space = gym.spaces.Box(
                low=-np.inf * np.ones(oracle_dim), high=np.inf * np.ones(oracle_dim)
            )
            obs_dict = {"state": state_space, "oracle_state": oracle_space}
            if self.use_history_obs:
                obs_dict.update(
                    {
                        "previous-state": state_space,
                        "previous-oracle_state": oracle_space,
                    }
                )
            for cam_name, cam_cfg in self.camera_infos.items():
                ...
                for modality_name in cam_cfg.keys():
                    ...
                    if modality_name == "rgb":
                        ...
                        spec = gym.spaces.Box(
                            low=0, high=MAX_DEPTH_RANGE, shape=resolution + (1,)
                        )
                        ...
                        obs_dict[f"{cam_name}-seg_gt"] = gym.spaces.Box(
                            low=-np.inf,
                            high=np.inf,
                            shape=(cam_cfg[modality_name]["num_points"],) + (4,),
                        )
                    elif modality_name == "segmentation":
                        spec = gym.spaces.Box(
                            low=0, high=255, shape=resolution + (2,), dtype=np.uint8
                        )
                    else:
                        raise RuntimeError(
                            "What happen? you should not see this error!"
            ...
            if len(self.imagination_infos) > 0:
                ...
                for img_name, points in self.imaginations.items():
                    ...
                    obs_dict[img_name] = gym.spaces.Box(
                        low=-np.inf, high=np.inf, shape=(num_points, 7)
                    )
                    if self.use_history_obs:
                        obs_dict["previous-" + img_name] = gym.spaces.Box(
                            low=-np.inf, high=np.inf, shape=(num_points, 7)
                        )
