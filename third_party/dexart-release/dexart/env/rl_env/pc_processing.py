...
def process_pc(
    ...
    within_bound = np.nonzero(
        np.logical_and.reduce(
            (within_bound_x, within_bound_y, within_bound_z, within_bound_r)
        )
    )[0]
    ...
    if num_index < num_points:
        indices = np.concatenate(
            [
                within_bound,
                np.ones(num_points - num_index, dtype=np.int32) * within_bound[0],
            ]
        )
        if noise_level != 0:
            multiplicative_noise = (
                1 + np_random.randn(num_index)[:, None] * 0.01 * noise_level
            )  # (num_index, 1)
            multiplicative_noise = np.concatenate(
                [
                    multiplicative_noise,
                    np.ones([num_points - num_index, 1]) * multiplicative_noise[0],
                ],
                axis=0,
            )
    else:
    ...
    if segmentation is not None:
        ...
        for instance_body_id in grouping_info["instance_body"]:
            instance_body_mask = np.logical_or(
                labels == instance_body_id, instance_body_mask
            )
        ...
        cloud = np.concatenate(
            [cloud, handle_mask, instance_body_mask, hand_mask, arm_mask], axis=1
        )
        # (N, 7) == (N, xyz + 4masks)
