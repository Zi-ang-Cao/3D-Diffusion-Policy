...
class ToiletEnv(BaseSimulationEnv):
    def __init__(
        ...
        cprint(
            f"[ToiletEnv] change_instance_when_reset={self.change_instance_when_reset}",
            "yellow",
        )
    ...
    def setup_instance_annotation(self):
        ...
        with open(self.joint_info_path) as f:
            ...
            for line in lines[1:]:
                ...
                self.joint_info[int(index)] = dict(
                    lid_joint_name=lid_joint_name,
                    lid_link_name=lid_link_name,
                    body_link_name=body_link_name,
                    seat_link_name=seat_link_name,
                )
    ...
    def load_instance(self, index):
        ...
        self.lid_link = [
            link
            for link in instance.get_links()
            if link.get_name() == self.joint_info[index]["lid_link_name"]
        ][0]
        if self.joint_info[index]["seat_link_name"] is not None:
            self.seat_link = [
                link
                for link in instance.get_links()
                if link.get_name() == self.joint_info[index]["seat_link_name"]
            ][0]
        ...
        q[revolute_joint_index] = (
            self.joint_limits_dict[str(index)]["left"] + self.init_open_rad
        )
