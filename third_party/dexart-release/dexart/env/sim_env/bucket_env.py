...
class BucketEnv(BaseSimulationEnv):
    def __init__(
        ...
        **renderer_kwargs,
        ...
        self.base_density = (
            renderer_kwargs["density"]
            if renderer_kwargs.__contains__("density")
            else 7800
        )
        ...
        cprint(
            f"[BucketEnv] change_instance_when_reset={self.change_instance_when_reset}",
            "yellow",
        )
        ...
        if not self.change_instance_when_reset:
            ...
            self.instance_base_link = [
                link
                for link in self.instance.get_links()
                if link.get_name() == "link_1"
            ][0]
    ...
    def setup_instance_annotation(self):
        ...
        for bucket_index in TASK_CONFIG["bucket"]:
            joint_json_path = (
                ...
                / "assets"
                / "sapien"
                / (str(bucket_index) + "_thick_handle")
                / "mobility_v2.json"
            )
    ...
    def load_instance(self, index):
        ...
        urdf_path = str(
            ...
            / "assets"
            / "sapien"
            / (str(index) + "_thick_handle")
            / "mobility.urdf"
        )

        loader.scale = self.scale_dict[str(index)]
        physical_material: sapien.PhysicalMaterial = (
        ...
        if self.joint_limits_dict[str(index)].__contains__("min"):
            ...
            q_limits[revolute_joint_index][0] = min(
                self.joint_limits_dict[str(index)]["min"],
                self.joint_limits_dict[str(index)]["max"],
            )
            q_limits[revolute_joint_index][1] = max(
                self.joint_limits_dict[str(index)]["min"],
                self.joint_limits_dict[str(index)]["max"],
            )
