...
from dexart.env.sim_env.constructor import (
    get_engine_and_renderer,
...
class BaseSimulationEnv(object):
    def __init__(
        ...
        **renderer_kwargs,
        ...
        engine, renderer = get_engine_and_renderer(
            use_gui=use_gui,
            need_offscreen_render=need_offscreen_render,
            no_rgb=no_rgb,
            **renderer_kwargs,
        )
    ...
    def create_camera_from_pose(
        ...
        if use_opencv_trans:
            ...
            pose_cam = pose
        cam = self.scene.add_camera(
            name, width=resolution[0], height=resolution[1], fovy=fov, near=0.1, far=10
        )
        cam.set_local_pose(pose_cam)
        self.cameras.update({name: cam})
