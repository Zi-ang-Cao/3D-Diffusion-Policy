...
def parse_args():
    ...
    parser.add_argument(
        "--num_episodes", type=int, default=10, help="number of total episodes"
    )
    parser.add_argument(
        "--use_test_set", dest="use_test_set", action="store_true", default=False
    )
    parser.add_argument(
        "--root_dir", type=str, default="data", help="directory to save data"
    )
    parser.add_argument("--img_size", type=int, default=84, help="image size")
    parser.add_argument(
        "--num_points", type=int, default=1024, help="number of points in point cloud"
    )
...
def main():
        ...
        while num_success < num_episodes:
            ...
            for j in range(env.horizon):
                ...
                if obs_point_cloud.shape[0] > args.num_points:
                    obs_point_cloud = downsample_with_fps(
                        obs_point_cloud, num_points=args.num_points
                    )
                ...
                if obs_image.shape[0] != args.img_size:
                    obs_image = (
                        F.interpolate(
                            torch.from_numpy(obs_image).permute(2, 0, 1).unsqueeze(0),
                            size=args.img_size,
                        )
                        .squeeze()
                        .permute(1, 2, 0)
    ...
    cprint(
        f"reward_mean = {np.mean(reward_list)}, success rate = {np.mean(success_list)}",
        "yellow",
    )
    ...
    imagin_robot_chunk_size = (
        env.horizon,
        imagin_robot_arrays.shape[1],
        imagin_robot_arrays.shape[2],
    )
    ...
    cprint(
        f"imagin_robot shape: {imagin_robot_arrays.shape}, range: [{np.min(imagin_robot_arrays)}, {np.max(imagin_robot_arrays)}]",
        "green",
    )
    cprint(
    ...
    cprint(
        f"depth shape: {depth_arrays.shape}, range: [{np.min(depth_arrays)}, {np.max(depth_arrays)}]",
        "green",
    )
    cprint(
