...
class WorldModel:
    def __init__(
        ...
        **kwargs,
        ...
        self.dynamics_net = DynamicsNet(
            state_dim, act_dim, hidden_size, residual=residual, seed=seed
        ).to(self.device)
        ...
        self.dynamics_opt = torch.optim.Adam(
            self.dynamics_net.parameters(), lr=fit_lr, weight_decay=fit_wd
        )
        ...
        if self.learn_reward:
            ...
            self.reward_net = RewardNet(
                state_dim, act_dim, hidden_size=(100, 100), seed=seed
            ).to(self.device)
            self.reward_net.set_transformations()  # in case device is different from default, it will set transforms correctly
            if activation == "tanh":
                self.reward_net.nonlinearity = torch.tanh
            self.reward_opt = torch.optim.Adam(
                self.reward_net.parameters(), lr=fit_lr, weight_decay=fit_wd
            )
    ...
    def compute_loss(self, s, a, s_next):
        ...
        sp = self.forward(s, a)
        s_next = (
            torch.from_numpy(s_next).float() if type(s_next) == np.ndarray else s_next
        )
    ...
    def fit_dynamics(
        ...
        **kwargs,
        ...
        if set_transformations:
            ...
            s_scale, a_scale = torch.mean(torch.abs(s - s_shift), dim=0), torch.mean(
                torch.abs(a - a_shift), dim=0
            )
            out_shift = (
                torch.mean(sp - s, dim=0)
                if self.dynamics_net.residual
                else torch.mean(sp, dim=0)
            )
            out_scale = (
                torch.mean(torch.abs(sp - s - out_shift), dim=0)
                if self.dynamics_net.residual
                else torch.mean(torch.abs(sp - out_shift), dim=0)
            )
    ...
    def fit_reward(
        ...
        **kwargs,
        ...
        if not self.learn_reward:
            print(
                "Reward model was not initialized to be learnable. Use the reward function from env."
            )
            return None
        ...
        assert (
            len(r.shape) == 2 and r.shape[1] == 1
        )  # r should be a 2D tensor, i.e. shape (N, 1)
        ...
        if set_transformations:
            ...
            r_shift, r_scale = torch.mean(r, dim=0), torch.mean(
                torch.abs(r - r_shift), dim=0
            )
            self.reward_net.set_transformations(
                s_shift, s_scale, a_shift, a_scale, r_shift, r_scale
            )
...
class DynamicsNet(nn.Module):
    ...
    def set_transformations(
        ...
        self.out_shift, self.out_scale = self.out_shift.to(device), self.out_scale.to(
            device
        )
    ...
    def set_params(self, new_params):
        ...
        s_shift, s_scale, a_shift, a_scale, out_shift, out_scale = new_params[
            "transforms"
        ]
...
class RewardNet(nn.Module):
    ...
    def set_transformations(
        ...
        if s_shift is None:
            self.s_shift, self.s_scale = torch.zeros(self.state_dim), torch.ones(
                self.state_dim
            )
            self.a_shift, self.a_scale = torch.zeros(self.act_dim), torch.ones(
                self.act_dim
            )
            self.sp_shift, self.sp_scale = torch.zeros(self.state_dim), torch.ones(
                self.state_dim
            )
            ...
            self.s_shift, self.s_scale = (
                torch.from_numpy(s_shift).float(),
                torch.from_numpy(s_scale).float(),
            )
            self.a_shift, self.a_scale = (
                torch.from_numpy(a_shift).float(),
                torch.from_numpy(a_scale).float(),
            )
            self.sp_shift, self.sp_scale = (
                torch.from_numpy(s_shift).float(),
                torch.from_numpy(s_scale).float(),
            )
            self.out_shift, self.out_scale = out_shift, out_scale
        ...
        self.s_shift, self.s_scale = self.s_shift.to(device), self.s_scale.to(device)
        self.a_shift, self.a_scale = self.a_shift.to(device), self.a_scale.to(device)
        self.sp_shift, self.sp_scale = self.sp_shift.to(device), self.sp_scale.to(
            device
        )
    ...
    def get_params(self):
        network_weights = [p.data for p in self.parameters()]
        transforms = (self.s_shift, self.s_scale, self.a_shift, self.a_scale)
        return dict(weights=network_weights, transforms=transforms)
...
def fit_model(nn_model, X, Y, optimizer, loss_func, batch_size, epochs, max_steps=1e10):
    """
