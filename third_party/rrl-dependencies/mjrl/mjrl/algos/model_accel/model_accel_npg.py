...
class ModelAccelNPG(NPG):
    def __init__(
        ...
        **kwargs,
        ...
        self.reward_function, self.termination_function = (
            reward_function,
    ...
    def train_step(
        ...
        **kwargs,
        ...
        reward_function = (
            self.reward_function if reward_function is None else reward_function
        )
        termination_function = (
            self.termination_function
            if termination_function is None
            else termination_function
        )
        if reward_function:
            assert callable(reward_function)
        if termination_function:
            assert callable(termination_function)
        ...
        init_states = (
            np.array([env.reset() for _ in range(N)])
        ...
        if truncate_lim is not None and len(self.learned_model) > 1:
            for path in paths:
                ...
                if truncated:
                    path["rewards"][-1] += truncate_reward
