...
for outer_iter in range(job_data["num_iter"]):
    ...
    samples_to_collect = (
        job_data["init_samples"] if outer_iter == 0 else job_data["iter_samples"]
    )
    iter_paths = sampler.sample_data_batch(
        samples_to_collect,
        agent.env,
        agent.policy,
        eval_mode=False,
    ...
    for i, model in enumerate(models):
        loss_general = model.compute_loss(
            s[-samples_to_collect:], a[-samples_to_collect:], sp[-samples_to_collect:]
    ...
    for inner_step in range(job_data["inner_steps"]):
        if job_data["start_state"] == "init":
            ...
            buffer_rand_idx = np.random.choice(
                len(init_states_buffer), size=job_data["update_paths"], replace=True
            ).tolist()
            ...
            if "buffer_frac" in job_data.keys():
                ...
                num_states_2 = (
                    int(job_data["update_paths"] * job_data["buffer_frac"]) + 1
                )
            else:
            ...
            buffer_rand_idx = np.random.choice(
                s.shape[0], size=num_states_2, replace=True
            )
            init_states_2 = list(s[buffer_rand_idx])
            init_states = init_states_1 + init_states_2

        agent.train_step(
            N=len(init_states), init_states=init_states, horizon=job_data["horizon"]
        )
        print_data = sorted(
            filter(
                lambda v: np.asarray(v[1]).size == 1,
