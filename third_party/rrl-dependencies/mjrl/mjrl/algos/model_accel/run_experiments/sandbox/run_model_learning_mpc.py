...
parser = argparse.ArgumentParser(
    description="Trajectory Optimization with fitted models."
)
parser.add_argument(
    "--output", type=str, required=True, help="location to store results"
)
parser.add_argument(
    "--config", type=str, required=True, help="path to config file with exp params"
)
...
SEED = job_data["seed"]
job_data["filter_coefs"] = [
    job_data["filter_coefs"][k] for k in ["f1", "f2", "f3", "f4"]
]
...
for outer_iter in range(job_data["num_iter"]):
    ...
    if outer_iter == 0:
        iter_paths = trajectory_sampler.sample_paths(
            job_data["n_init_paths"],
            e,
            exploratory_policy,
            eval_mode=False,
            base_seed=SEED,
        ...
        iter_paths = sample_paths(
            job_data["paths_per_iter"],
            mpc_policy.env,
            mpc_policy,
    ...
    exp_data = dict(
        policy=mpc_policy,
        fitted_model=mpc_policy.fitted_model,
        log=logger.log,
        rollout_paths=iter_paths,
        eval_paths=eval_paths,
    )
    if outer_iter > 0 and outer_iter % job_data["save_freq"] == 0:
        pickle.dump(exp_data, open(PICKLE_FILE, "wb"))
        pickle.dump(
            exp_data, open(OUT_DIR + "/iteration_" + str(outer_iter) + ".pickle", "wb")
        )
