...
def main(args: DictConfig):
    ...
    if job_data["from_pixels"] == True:
        ...
        e, env_kwargs = make_env(
            job_data["env"],
            from_pixels=True,
            cam_list=cam_list,
            encoder_type=job_data["encoder_type"],
            hybrid_state=job_data["hybrid_state"],
        )
    else:
    ...
    if job_data["algorithm"] != "NPG":
        ...
        if job_data["eval_rollouts"] >= 1:
            score = e.evaluate_policy(
                policy, num_episodes=job_data["eval_rollouts"], mean_action=True
            )
            print("Score with behavior cloning = %f" % score[0][0])
            try:
                print("Number of cpu: ", job_data["num_cpu"])
                eval_paths = sample_paths(
                    num_traj=job_data["eval_rollouts"],
                    policy=policy,
                    num_cpu=job_data["num_cpu"],
                    env=e.env_id,
                    eval_mode=True,
                    base_seed=job_data["seed"],
            ...
            pickle.dump(
                ...
                open(
                    JOB_DIR
                    + "/demo_bs{}_epochs{}.pickle".format(
                        job_data["bc_batch_size"], job_data["bc_epochs"]
                    ),
                    "wb",
                ),
            )
