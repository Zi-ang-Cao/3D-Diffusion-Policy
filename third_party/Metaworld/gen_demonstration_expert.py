...
def main(args):
    ...
    save_dir = os.path.join(
        args.root_dir, "metaworld_" + args.env_name + "_expert.zarr"
    )
    ...
    while episode_idx < num_episodes:
        ...
        if not ep_success or ep_success_times < 5:
            ...
            cprint(
                "Episode: {}, Reward: {}, Success Times: {}".format(
                    episode_idx, ep_reward, ep_success_times
                ),
                "green",
            )
            episode_idx += 1
    ...
    cprint(
        f"full_state shape: {full_state_arrays.shape}, range: [{np.min(full_state_arrays)}, {np.max(full_state_arrays)}]",
        "green",
    )
    cprint(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--root_dir", type=str, default="../../3D-Diffusion-Policy/data/"
    )
